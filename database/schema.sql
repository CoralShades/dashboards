-- Centralized BI Dashboard Portal Database Schema
-- This script creates the necessary tables for the BI dashboard portal
-- Based on PRD and Architecture requirements

-- Create profiles table to link users to roles
...
-- Financial data tables for Xero integration
...
-- RLS Policies for data access based on user roles
-- Admin can see all data
CREATE POLICY "Admin can view all financial data" ON public.weekly_income
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admin can view all wages data" ON public.weekly_wages
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admin can view all expenses data" ON public.weekly_expenses
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admin can view all WIP projects" ON public.wip_projects
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
-- Role B (Finance & HR) can see financial data
CREATE POLICY "Role B can view financial data" ON public.weekly_income
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role IN ('role_b', 'admin')
        )
    );

CREATE POLICY "Role B can view wages data" ON public.weekly_wages
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role IN ('role_b', 'admin')
...
CREATE POLICY "Role B can view expenses data" ON public.weekly_expenses
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role IN ('role_b', 'admin')
...
-- Role A (Sales & Marketing) can see limited project data for sales insights
CREATE POLICY "Role A can view basic project data" ON public.wip_projects
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role IN ('role_a', 'admin')
        )
    );
...
BEGIN
    ...
    VALUES (NEW.id, 'role_a', NEW.raw_user_meta_data->>'full_name');
    RETURN NEW;
END;
